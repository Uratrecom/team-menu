...
function Language.ParseLanguage(lang)
    if not Utils.CheckArguments(lang) then
        return
    end
...
Utils.AddArguments(Language.ParseLanguage, {
    { name = "lang", required = true, type = "string", validator = Utils.LanguageValidator },
})

function Language.RefreshLanguage(lang)
    if not Utils.CheckArguments(lang) then
        return
    end
...
Utils.AddArguments(Language.RefreshLanguage, {
    { name = "lang", required = true, type = "string", validator = Utils.LanguageValidator },
})

function Language.RefreshAll()
    for lang in pairs(Language.languages) do
        cache[lang] = language.ParseLanguage(lang)
    end
end